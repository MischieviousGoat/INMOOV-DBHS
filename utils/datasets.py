...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            hsvFrame = cv2.cvtColor(self.imgs[i], cv2.COLOR_BGR2HSV)
            ...
            # between self.imgs[i] and mask determines 
            # to detect only that particular color 
            kernel = np.ones((5, 5), "uint8") 
            
            # # For red color 
            # red_mask = cv2.dilate(red_mask, kernel) 
            # res_red = cv2.bitwise_and(self.imgs[i], self.imgs[i], ask = red_mask) 
            
            # # For green color 
            # green_mask = cv2.dilate(green_mask, kernel) 
            # res_green = cv2.bitwise_and(self.imgs[i], self.imgs[i], mask = green_mask) 
            
            # # For blue color 
            # blue_mask = cv2.dilate(blue_mask, kernel) 
            # res_blue = cv2.bitwise_and(self.imgs[i], self.imgs[i], mask = blue_mask) 
            ...
            for pic, contour in enumerate(contours): 
                ...
                if(area > 300): 
                    ...
                    self.imgs[i] = cv2.rectangle(self.imgs[i], (x, y),  
                                            (x + w, y + h),  
                                            (0, 0, 255), 2) 
                    
                    cv2.putText(self.imgs[i], "Red Colour", (x, y), 
            ...
            for pic, contour in enumerate(contours): 
                ...
                if(area > 300): 
                    ...
                    cv2.putText(self.imgs[i], "Green Colour", (x, y), 
            ...
            for pic, contour in enumerate(contours): 
                ...
                if(area > 300): 
                    ...
                    cv2.putText(self.imgs[i], "Blue Colour", (x, y), 
                                cv2.FONT_HERSHEY_SIMPLEX, 
                                1.0, (255, 0, 0)) 
